...
altura <- subset(quest_21, altura >= 100 & altura <= 250)
...
peso <- subset(quest_21, peso >= 15 & peso <= 500)
...
<center>
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=TRUE, echo = FALSE}
quest_21$nascimento <- as.Date(quest_21$nascimento, "%d/%m/%Y")

quest_21$idade <- floor(difftime(Sys.Date(), quest_21$nascimento, units = "weeks")/52)

quest_21$idade <- as.integer(quest_21$idade)

idade <- subset(quest_21, idade >= 15 & idade <= 100)
...
quest_20$dataNasc <- as.Date(quest_20$dataNasc, "%d/%m/%Y")

quest_20$idade <- (floor(difftime(Sys.Date(), quest_20$dataNasc, units = "weeks")/52)-1)

quest_20$idade <- as.integer(quest_20$idade)

idade_20 <- subset(quest_20, idade >= 15 & idade <= 100)


quest_19$dataNasc <- as.Date(quest_19$dataNasc, "%d/%m/%Y")

quest_19$idade <- (floor(difftime(Sys.Date(), quest_19$dataNasc, units = "weeks")/52)-2)

quest_19$idade <- as.integer(quest_19$idade)

idade_19 <- subset(quest_19, idade >= 15 & idade <= 100)



idade <- data.frame(idade = c(idade_19$idade,
                             idade_20$idade,
                            idade_21$idade),
                   ano = c(rep(2019,nrow(idade_19)),
                           rep(2020,nrow(idade_20)),
                           rep(2021,nrow(idade_21))))
...
<center>
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=TRUE, echo = FALSE}
mes <- month(as.POSIXlt(quest_21$nascimento, format="%d-%m-%Y"))
...
irmaos <- subset(quest_21, irmaos >= 0)
...
tempo_ies <- subset(quest_21, tempo_ies >= 0 & tempo_ies<=700)
...
app_transporte <- subset(quest_21, app_transporte >= 0 & app_transporte<=700)
...
atv_fisica_n <- subset(quest_21, atv_fisica_n >= 0 & atv_fisica_n<=7)
