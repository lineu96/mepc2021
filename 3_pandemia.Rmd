...
### 2021
quest_21$covid_estudos <- tolower(iconv(quest_21$covid_renda,
                                   to ='ASCII//TRANSLIT', 
                                   from = "UTF-8"))
...
### Como você avalia as medidas tomadas pelas autoridades para evitar o colapso do sistema de saúde? {.tabset .tabset-fade .tabset-pills}

---

```{r, echo=FALSE}
## 2021
quest_21$covid_colapso <- tolower(iconv(quest_21$covid_colapso,
                                      to ='ASCII//TRANSLIT', 
                                      from = "UTF-8"))

quest_21$covid_colapso <- factor(quest_21$covid_colapso, 
                               levels = tolower(c("Suficientes", 
                                                  "Insuficientes", 
                                                  "Não sei dizer")))
...
table1 <- freq_ord(quest_21$covid_colapso)
```
...
ggplot(data=table1, aes(x=reorder(Niveis, -Frequencia), 
  ...
  ylab("Frequência") +
  xlab("") +
  ggtitle("Medidas tomadas pelas autoridades")+
  theme_classic() + theme(legend.position = 'none',
    plot.title = element_text(family = "Helvetica", 
                              face = "bold", 
                              size = (20),
