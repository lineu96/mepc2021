...
Em uma análise exploratória buscamos sintetizar a informação presente nos dados, em geral por meio de tabelas de frequência e gráficos.
...
Algumas possibilidades de gráficos para cada tipo de variável são:
 ...
 - **Quantitativa discreta**: barras, linhas, histograma, boxplot.
...
Considere um conjunto de dados genérico, composto por uma variável quantitativa discreta, duas variáveis quantitativas contínuas e uma variável qualitativa nominal. Veja alguns exemplos das representações mencionadas:
...
<center>
```{r, echo=FALSE}
pander(freq_n_ord(df$categorica1))
```
</center>
...
<center>
```{r, echo=FALSE}
pander(tab_classes(df$continua1))
```
</center>
...
<center>
```{r, echo=FALSE}
pander(posicao(df$continua1))
pander(dispersao(df$continua1))
```
</center>
